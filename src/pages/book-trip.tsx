...
const BookTrip = () => {
  ...
  const getRecurrenceBookings = async (transaction: Transaction): Promise<DateCarBooking[]> => {
      ...
      await runTransaction(db, async (transaction) => {
        if (recurrenceId && !single) {
          ...
          // Must fetch all real DateCarBookings before updating/deleting all of them.
           const recurrenceBookings = await getRecurrenceBookings(transaction);
          recurrenceBookings.forEach(book => {
            ...
            updateOrDeleteDateBooking(transaction, doc(db, 'date-car-bookings', book.id), bookings.filter(
                b => b.recurrenceId !== recurrenceId || b.logged
            ));
          });
           // Keep the recurrence document because historical/logged bookings may
           // still reference it after the unlogged future bookings are removed.
        } else {
          ...
          if (dateBooking) {
            ...
            updateOrDeleteDateBooking(transaction, doc(db, 'date-car-bookings', dateBooking.id), dateBookingsDoc.data().bookings.filter(
                b => b.id !== existingBooking || b.logged
            ));
          }
        }
      });
