...
import {collection, doc, addDoc, updateDoc, getDoc, serverTimestamp, runTransaction} from 'firebase/firestore';
  ...
  useEffect(() => {
    ...
    for (const dcb of recentBookings) {
      ...
      if (candidates.length > 0) {
        ...
        const augmented = { ...chosen, car: { id: selectedCar }, date: dcb.date, parent_id: dcb.id };
        setIsConnectedBooking(true);
      ...
      if (isEditMode && lastTrip) {
       ...
       if (isConnectedBooking && connectedBooking) {
         ...
         if (recurrenceId) {
           ...
           isMultiDay = recurrenceDoc.exists() && recurrenceDoc.data().isMultiDay === true;
         }

         if (recurrenceId && isMultiDay) {
           // Multi-day booking: mark all bookings in the multi-day sequence.
           ...
           // Single or recurring booking: mark only this specific booking.
